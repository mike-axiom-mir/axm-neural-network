...
**Repository context:** neural substrate, backend and compute-independence research  
...
This repository is the **substrate and independence path**. Its job is to understand, compare and progressively own the mathematics and execution layers beneath AXM brains.

Near-term emphasis:
- treat PyTorch/JAX/MLX as reference calculators/backends, not architectural owners;
- define a backend-neutral AXM learning contract;
- compare numerical and behavioral results across backends;
- identify the smallest tensor/autodiff/update subset AXM actually needs;
- build an AXM-owned CPU path before chasing custom accelerators;
- move lower only when evidence shows the next dependency is worth replacing.
